...
import {
  MapPin, CheckCircle2, XCircle, Clock, TrendingUp, Building2,
  Users, LogOut, ShieldCheck, ChevronDown
} from "lucide-react";
...
export default function AdminDashboard() {
  ...
  const updateStatus = useMutation({
    mutationFn: ({ id, status }: { id: number; status: string }) =>
      api.admin.updateStatus(id, status),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["admin-businesses"] });
      qc.invalidateQueries({ queryKey: ["admin-stats"] });
      toast({ title: "Status updated", description: "Business status has been updated." });
    },
    onError: (err: Error) => {
    ...
    <div className="min-h-screen bg-background font-sans text-foreground">
      ...
      <main className="container mx-auto px-4 md:px-6 py-10 max-w-6xl">
        <div className="mb-10">
          ...
          <p className="text-muted-foreground">Manage businesses, sellers, and society listings</p>
        </div>
        ...
        <h2 className="text-xl font-bold text-foreground mb-5">Business Approvals</h2>
                        ...
                        <div className="flex items-start justify-between gap-4 flex-wrap">
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2 mb-1 flex-wrap">
                              <h3 className="font-bold text-foreground">{biz.businessName}</h3>
                              <Badge className={`text-xs ${STATUS_COLORS[biz.status]}`}>
                                {biz.status}
                              </Badge>
                            ...
                            <div className="flex items-center gap-4 text-xs text-muted-foreground flex-wrap">
                              ...
                              <span>Society: <strong>{row.society?.name ?? "—"}</strong></span>
                          ...
                          <div className="flex gap-2 shrink-0">
                            {biz.status !== "approved" && (
                              <Button
                                ...
                                onClick={() => updateStatus.mutate({ id: biz.id, status: "approved" })}
                              >
